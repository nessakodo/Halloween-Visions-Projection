...
def main():
    ...
    if args.sequence in ["sleep", "both"]:
        print("📤 Sending: /sequences/sleepseq/play 1.0")
        client.send_message("/sequences/sleepseq/play", 1.0)
        if args.sequence == "both":
            print(f"⏱️  Waiting {args.delay}s...")
            time.sleep(args.delay)
    
    if args.sequence in ["scare", "both"]:
        print("📤 Sending: /sequences/scareseq/play 1.0")
        client.send_message("/sequences/scareseq/play", 1.0)
